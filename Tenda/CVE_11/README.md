...
the `led_ctl_type` variable can be controlled by attacker by calling the function `sub_800D50A4`(page `/goform/setLED`) which gets in a parameter called `LEDStatus` into varialbe `v8`, which can then be used to assign to the `led_ctl_type` variable.If `v8` is different from the original value of nvram variable `led_ctl_type`,then the nvram variable `led_ctl_type` will be set as `v8`
...
Credit to [@Yu3H0](https://github.com/Yu3H0), [@leonW7](https://github.com/leonW7), and [@Lnkvct](https://github.com/Lnkvct) from Shanghai Jiao Tong University.
